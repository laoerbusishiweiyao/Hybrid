...
$EdgeServerOutput = "../../$Configuration/EdgeServer"
$EdgeServerOpenApiFilesCache = "../../Server/EdgeServer/bin/$Configuration/$TargetFramework/obj"
...
if (Test-Path $EdgeServerOutput) {
    Write-Host "🧹 正在清空目录..." -ForegroundColor Cyan
    Remove-Item $EdgeServerOutput -Recurse -Force
    Write-Host "✅ 目录已清空。" -ForegroundColor Green
}

Write-Host "📦 正在构建 Edge Server..." -ForegroundColor Cyan

if (-not (Test-Path $EdgeServerOpenApiFilesCache)) {
    New-Item -ItemType Directory -Force -Path $EdgeServerOpenApiFilesCache | Out-Null
}

Push-Location -Path "../../Server/EdgeServer"
dotnet restore
dotnet publish -c $Configuration -o $EdgeServerOutput
Pop-Location

Write-Host "✅ Edge Server 构建完成。" -ForegroundColor Green