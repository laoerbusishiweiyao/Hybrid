...
[Preserve]
public sealed class MemoryPackComponentCollectionFormatter : MemoryPackFormatter<ComponentCollection>
{
    [Preserve]
    public override void Serialize<TBufferWriter>(ref MemoryPackWriter<TBufferWriter> writer, scoped ref ComponentCollection value)
    {
        if (value == null)
        {
    ...
    [Preserve]
    public override void Deserialize(ref MemoryPackReader reader, scoped ref ComponentCollection value)
    {
        ...
        {
            // 这里甚至可以用对象池
            value = ComponentCollection.Create(true);
        }
        else
        {
            value.Clear();
