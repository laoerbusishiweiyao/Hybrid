...
using static Chaos.IQUEUEHEAD;
using static Chaos.KCPBASIC;
...
namespace Chaos
{
    internal static unsafe class KcpBridge
    {
        ...
        private static void* malloc(nint size) => (void*)Marshal.AllocHGlobal(size);

        private static void* malloc(nuint size) => (void*)Marshal.AllocHGlobal((nint)size);

        private static void free(void* ptr) => Marshal.FreeHGlobal((nint)ptr);
