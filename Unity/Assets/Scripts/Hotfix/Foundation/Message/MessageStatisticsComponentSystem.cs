...
namespace Chaos
{
    ...
    public static partial class MessageStatisticsComponentSystem
    {
        ...
        {
            self.CurrentSecondStart = TimeInfo.Default.ClientNow();
        }
        ...
        public static bool Check(this MessageStatisticsComponent self, Type messageType, int maxMessageCount)
        {
            ...
            // 累加消息计数
            var count = self.MessageStats.GetValueOrDefault(messageType, 0);
        ...
        {
            var stats = self.MessageStats;

            var builder = new StringBuilder();
            builder.AppendLine("========== Message Statistics ==========");
            builder.AppendLine($"Window: Current Second ({self.WindowSize}ms)");
            builder.AppendLine($"Total Messages: {self.TotalCount}, MaxMessageCount: {maxMessageCount}");
            ...
            if (stats.Count > 0)
            {
                ...
                {
                    builder.AppendLine($"  {kvp.Key.FullName}: {kvp.Value}");
                }
            }
            else
            {
                builder.AppendLine("No messages in current second");
            }

            builder.AppendLine("========================================");
            Log.Information("{builder}", builder);
        }
    }
}